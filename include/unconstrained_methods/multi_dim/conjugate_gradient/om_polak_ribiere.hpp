#if !defined(OM_POLAK_RIBIERE)
#define OM_POLAK_RIBIERE
...
/**
 * @brief Polak-Ribiere method object
 *
 * @tparam fp_type fp_type is a floating-point template parameter
 */
template <typename fp_type = double>
class polak_ribiere_method : public conjugate_gradient_base<fp_type> {
public:
  /**
   * @brief Construct a new polak ribiere method object
   *
...
om_unconstrained_methods::om_conjugate_gradient::polak_ribiere_method<fp_type>::
  ...
  while (iters < this->max_iters_) {
    ...
    if (((x - x_prev).norm() < this->arg_tol_) &&
      ...
      beta =
          (grad - grad_prev).dot(grad) / (grad_prev.norm() * grad_prev.norm());
