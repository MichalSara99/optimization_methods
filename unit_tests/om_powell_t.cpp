...
#ifdef _DEBUG_
  std::cout << "================== Powell Method (Default) ==============\n";
...
BOOST_AUTO_TEST_CASE(powell_float_0) {

  auto minF = [](float x) { return (x * x + 2.0 * std::exp(-1.0 * x)); };
  auto r = range<float>{0.0, 2.0};
  std::size_t max_iters = 1000;
  float tol = 10e-5;
  powell_method<float> g{r, tol, max_iters};
  auto g_min = g(minF);
...
BOOST_AUTO_TEST_CASE(powell_default_1) {
...
BOOST_AUTO_TEST_CASE(powell_float_1) {
