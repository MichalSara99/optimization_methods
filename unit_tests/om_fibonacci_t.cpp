...
#ifdef _DEBUG_
  std::cout << "================== Fibonacci Method (Default) ==============\n";
...
BOOST_AUTO_TEST_CASE(fibonacci_float_0) {
  auto minF = [](float x) { return (x * x + 2.0 * std::exp(-1.0 * x)); };
  auto r = range<float>{0.0, 2.0};
  float tol = 10e-5;
  std::size_t max_iters = 1000;
  fibonacci_method<float> g{r, tol, max_iters};
  auto g_min = g(minF);
...
#ifdef _DEBUG_
  std::cout << "================== Fibonacci Method (Float) ==============\n";
  std::cout << "minimiser: " << std::get<0>(g_min) << "\n"
            << "fun value: " << std::get<1>(g_min) << "\n"
            << "fun evaluations: " << std::get<2>(g_min) << "\n"
            << "iterations: " << std::get<3>(g_min) << "\n";
...
BOOST_AUTO_TEST_CASE(fibonacci_default_1) {
...
BOOST_AUTO_TEST_CASE(fibonacci_float_1) {
